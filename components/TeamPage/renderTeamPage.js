...
export default function () {
    ...
    const readDocData = async () => {
        try {
            // 3
            const querySnapshot = await getDocs(collection(firestore, "teams")); // to jest jeszcze lista opakowanych dokumentów w kolekcji
            // 4
            querySnapshot.forEach((doc) => {
                const todo = doc.data(); // metoda data rozpakowuje obiekt .json i w todo jest on już w postaci .js
                // 5
                console.log(todo);
                // 6
